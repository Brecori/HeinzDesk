...
function fetchAndCreateCards() {
  ...
  fetch(baseUrl)
    ...
    .then(data => {
      ...
      const dataFiltrado = data.filter(response => response.answered === true).filter(response => response.typeEsg === 'G');
      ...
      dataFiltrado.forEach(response => {
        ...
        link.href = `./comentariodiretoG.html?personId=${response.personId}`;
